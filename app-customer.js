...
function startServer(portToUse, retryCount = 0) {
    const maxRetries = 5;
    logger.info(`Mencoba memulai server pada port ${portToUse} (Percobaan ${retryCount + 1}/${maxRetries})...`);
    ...
    try {
        const server = app.listen(portToUse, '0.0.0.0', () => {
            ...
            if (err.code === 'EADDRINUSE') {
                logger.warn(`PERINGATAN: Port ${portToUse} sedang digunakan.`);
                if (retryCount < maxRetries - 1) {
                    logger.info(`Menunggu 2 detik sebelum mencoba lagi...`);
                    setTimeout(() => {
                        startServer(portToUse, retryCount + 1);
                    }, 2000);
                } else {
                    logger.error(`ERROR: Port ${portToUse} tetap digunakan setelah ${maxRetries} kali mencoba. Keluar.`);
                    process.exit(1);
                }
            } else {
