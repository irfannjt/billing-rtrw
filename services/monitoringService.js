...
function getHealthStatus() {
  ...
  // Check VPN / WireGuard Health
  try {
    const vpnHealth = require('./vpnHealthService');
    const vpnStatus = vpnHealth.getLatestStatus();
    if (vpnStatus && vpnStatus.configured) {
      if (!vpnStatus.online) {
        warnings.push(`Koneksi VPN / WireGuard ke MikroTik Terputus: ${vpnStatus.error || 'RTO'}`);
      }
    }
  } catch (e) {
    // ignore
