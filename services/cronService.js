...
function startCronJobs() {
  ...
  // 9. VPN & WireGuard Health Check - Setiap 5 Menit (atau sesuai config)
  cron.schedule('*/5 * * * *', async () => {
    try {
      const vpnHealth = require('./vpnHealthService');
      const config = vpnHealth.getVPNSettings();
      if (!config.enabled) return;

      logger.info('[CRON] Menjalankan pengecekan kesehatan VPN/WireGuard...');
      await vpnHealth.checkPing();
    } catch (e) {
      logger.error(`[CRON] Error VPN Health Check: ${e.message}`);
    }
  });
