...
/**
 * Marks a resident's bill (invoice) as 'Paid' and stores the payment timestamp in the database.
 * @param {number|string} invoiceId - The ID of the resident's bill (invoice)
 * @param {string} [paidByName='Admin'] - Name of the collector, cashier, or admin who processed the payment
 * @param {string} [notes=''] - Additional payment notes
 * @param {object} [actor=null] - Actor object for audit trail logs
 * @returns {object} Database run result
 */
function markResidentBillAsPaid(invoiceId, paidByName = 'Admin', notes = '', actor = null) {
  ...
  // We reuse markAsPaid which sets status='paid' and paid_at=NOW_LOCAL() (the local timezone payment timestamp)
  return markAsPaid(id, paidByName, notes, actor);
}
...
function getBillingDashboardStats(year) {
...
function getOutstandingResidentsSummary() {
  return db.prepare(`
    SELECT c.id as customer_id, c.name, c.phone, c.status as customer_status,
           COUNT(*) as unpaid_count, SUM(i.amount) as total_unpaid,
           GROUP_CONCAT(i.id, ',') as invoice_ids,
           GROUP_CONCAT(i.period_month || '/' || i.period_year, ', ') as unpaid_periods
    FROM invoices i JOIN customers c ON i.customer_id = c.id
    WHERE i.status='unpaid'
    GROUP BY c.id ORDER BY total_unpaid DESC, unpaid_count DESC
  `).all();
}
...
module.exports = {
  ...
  markAsPaid, markResidentBillAsPaid, markAsUnpaid, deleteInvoice,
  getInvoiceSummary, getMonthlyRevenue,
  getDashboardStats, getRecentPayments, getRecentTransactions, getTopUnpaid,
  getTodayRevenue,
  updatePaymentInfo,
  getBillingDashboardStats,
  getOutstandingResidentsSummary
};
