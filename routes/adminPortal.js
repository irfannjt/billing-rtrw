...
const vpnClientService = require('../services/vpnClientService');
...
router.get('/', requireAdminSession, requireSidebarMenuAccess('dashboard'), async (req, res) => {
  try {
    ...
    const recentPayments = billingSvc.getRecentPayments(15); // Get up to 15 recent payment transactions
    const recentTransactions = billingSvc.getRecentTransactions(20); // Get up to 20 recent transactions (all statuses)
    
    // Get Network Stats (Routers, VPN)
    const routers = mikrotikService.getAllRouters() || [];
    const activeRoutersCount = routers.filter(r => r.is_active === 1 || r.is_active === '1' || r.is_active === true).length;
    const vpnClients = vpnClientService.getAllClients() || [];
    const vpnClientsCount = vpnClients.length;
    const vpnHealthService = require('../services/vpnHealthService');
    const vpnServerStatus = vpnHealthService.getLatestStatus();
    
    res.render('admin/dashboard', {
      title: 'Dashboard', company: company(), version: '2.0.0',
      activePage: 'dashboard', billing, custStats, settings, recentPayments, recentTransactions,
      networkStats: {
        totalRouters: routers.length,
        activeRouters: activeRoutersCount,
        totalVpnClients: vpnClientsCount,
        vpnServer: vpnServerStatus
      }
    });
  } catch (e) {
...
router.get('/billing-dashboard', requireAdminSession, requireSidebarMenuAccess('billing_dashboard'), (req, res) => {
  const filterYear = parseInt(req.query.year) || new Date().getFullYear();
  const stats = billingSvc.getBillingDashboardStats(filterYear);
  const outstandingResidents = billingSvc.getOutstandingResidentsSummary();
  
  res.render('admin/billing_dashboard', {
    title: 'Dashboard Tagihan',
    company: company(),
    activePage: 'billing_dashboard',
    stats,
...
router.get('/billing/:id/print', requireAdminSession, (req, res) => {
  ...
  res.render('admin/print_invoice', {
    invoice: inv,
    customer,
    company: settings.company_header || 'Billing Nusa Digital',
    settings
  });
});
...
router.post('/whatsapp/test-notification', requireAdminSession, async (req, res) => {
  try {
    ...
    const msg =
      ...
      `✅ Jika pesan ini masuk, berarti notifikasi WhatsApp dari Billing Nusa Digital System sudah berfungsi.\n` +
      `📅 Waktu: ${getNowLocal()}`;
...
// ==========================================
// VPN, RADIUS & WireGuard Health Check Routes
// ==========================================
const vpnHealthSvc = require('../services/vpnHealthService');

router.get('/vpn-health', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const vpnSettings = vpnHealthSvc.getVPNSettings();
    const vpnStatus = vpnHealthSvc.getLatestStatus();
    const pingHistory = vpnHealthSvc.getPingHistory();
    const routers = vpnHealthSvc.getAvailableRouters();
    
    const l2tpSettings = vpnHealthSvc.getL2TPSettings();
    const radiusSettings = vpnHealthSvc.getRADIUSSettings();
    const nasList = vpnHealthSvc.getNASList();
    
    const msg = req.session._msg || null;
    delete req.session._msg;

    res.render('admin/vpn_health', {
      title: 'RADIUS / NAS',
      activePage: 'vpn_health',
      company: getSetting('company_header', 'Billing Nusa Digital'),
      lang: req.session.lang || 'id',
      session: req.session,
      t: (key, fallback) => require('../config/i18n').t(req.session.lang || 'id', key, fallback),
      sidebarSections: sidebarMenuSvc.getSidebarSections(req.session),
      sidebarBottomNavItems: sidebarMenuSvc.getBottomNavItems(req.session),
      vpnSettings,
      vpnStatus,
      pingHistory,
      routers,
    ...
    logger.error('Error rendering VPN health check:', error);
    res.status(500).send('Error rendering VPN health check page');
  }
});

// Alias redirect
router.get('/radius-nas', (req, res) => res.redirect('/admin/vpn-health'));

router.post('/vpn-health/save', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const settings = {
      enabled: req.body.enabled === 'on' || req.body.enabled === 'true',
      vps_iface: req.body.vps_iface,
      vps_port: req.body.vps_port,
      vps_ip: req.body.vps_ip,
      vps_private_key: req.body.vps_private_key,
      vps_public_key: req.body.vps_public_key,
      vps_endpoint_ip: req.body.vps_endpoint_ip,
      mt_router_id: req.body.mt_router_id,
      mt_port: req.body.mt_port,
      mt_ip: req.body.mt_ip,
      mt_private_key: req.body.mt_private_key,
      mt_public_key: req.body.mt_public_key,
      keepalive: req.body.keepalive,
      cron_interval: req.body.cron_interval
    };

    const success = vpnHealthSvc.saveVPNSettings(settings);
    if (success) {
      req.session._msg = { type: 'success', text: 'Konfigurasi WireGuard berhasil disimpan.' };
    } else {
      req.session._msg = { type: 'error', text: 'Gagal menyimpan konfigurasi WireGuard.' };
    }
  } catch (error) {
    logger.error('Error saving VPN settings:', error);
    req.session._msg = { type: 'error', text: 'Kesalahan internal: ' + error.message };
  }
  res.redirect('/admin/vpn-health?tab=wireguard');
});

router.post('/vpn-health/save-l2tp', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const settings = {
      enabled: req.body.enabled === 'on' || req.body.enabled === 'true',
      vps_ip: req.body.vps_ip,
    ...
    const success = vpnHealthSvc.saveL2TPSettings(settings);
    if (success) {
      req.session._msg = { type: 'success', text: 'Konfigurasi L2TP/IPSec berhasil disimpan.' };
    } else {
      req.session._msg = { type: 'error', text: 'Gagal menyimpan konfigurasi L2TP/IPSec.' };
    }
  } catch (error) {
    logger.error('Error saving L2TP settings:', error);
    req.session._msg = { type: 'error', text: 'Kesalahan internal: ' + error.message };
  }
  res.redirect('/admin/vpn-health?tab=l2tp');
});

router.post('/vpn-health/save-radius', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const settings = {
      auth_pap: req.body.auth_pap === 'on' || req.body.auth_pap === 'true',
      auth_chap: req.body.auth_chap === 'on' || req.body.auth_chap === 'true',
      auth_mschap1: req.body.auth_mschap1 === 'on' || req.body.auth_mschap1 === 'true',
      auth_mschap2: req.body.auth_mschap2 === 'on' || req.body.auth_mschap2 === 'true',
      single_session: req.body.single_session === 'on' || req.body.single_session === 'true'
    };

    const success = vpnHealthSvc.saveRADIUSSettings(settings);
    if (success) {
      req.session._msg = { type: 'success', text: 'Konfigurasi Autentikasi RADIUS berhasil disimpan.' };
    } else {
      req.session._msg = { type: 'error', text: 'Gagal menyimpan konfigurasi RADIUS.' };
    }
  } catch (error) {
    logger.error('Error saving RADIUS settings:', error);
    req.session._msg = { type: 'error', text: 'Kesalahan internal: ' + error.message };
  }
  res.redirect('/admin/vpn-health?tab=nas');
});

router.post('/vpn-health/add-nas', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const nas = {
      name: req.body.name,
      ip_nas: req.body.ip_nas,
      type: req.body.type || 'PPPoE/Hotspot',
      secret: req.body.secret
    };

    const success = vpnHealthSvc.addNAS(nas);
    if (success) {
      req.session._msg = { type: 'success', text: 'NAS Baru berhasil ditambahkan.' };
    } else {
      req.session._msg = { type: 'error', text: 'Gagal menambahkan NAS.' };
    }
  } catch (error) {
...
router.post('/vpn-health/delete-nas/:id', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const success = vpnHealthSvc.deleteNAS(req.params.id);
    if (success) {
      req.session._msg = { type: 'success', text: 'NAS berhasil dihapus.' };
    } else {
      req.session._msg = { type: 'error', text: 'Gagal menghapus NAS.' };
    }
  } catch (error) {
...
router.post('/vpn-health/ping', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const result = await vpnHealthSvc.checkPing();
    res.json({ success: true, ...result });
  } catch (error) {
    logger.error('Error checking ping:', error);
    res.json({ success: false, error: error.message });
  }
});

router.post('/vpn-health/generate-keys', requireAdminSession, restrictToAdmin, (req, res) => {
  try {
    const privateKey = vpnHealthSvc.generateWireGuardKey();
    const publicKey = vpnHealthSvc.generateWireGuardKey();
...
router.get('/vpn-health/detect-ip', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    const ip = await vpnHealthSvc.detectPublicIP();
...
router.get('/vpn-clients/generate-wg-keys', requireAdminSession, restrictToAdmin, (req, res) => {
  try {
    const serverPriv = vpnClientSvc.generateWireGuardKey();
    const serverPub = vpnClientSvc.generatePublicKey(serverPriv);
    const clientPriv = vpnClientSvc.generateWireGuardKey();
    const clientPub = vpnClientSvc.generatePublicKey(clientPriv);
    res.json({
      server: { privateKey: serverPriv, publicKey: serverPub },
      client: { privateKey: clientPriv, publicKey: clientPub }
    });
  } catch (error) {
    console.error(error);
...
router.get('/vpn-clients', requireAdminSession, restrictToAdmin, async (req, res) => {
  try {
    ...
    const user = {
      role: req.session.isCashier ? 'Kasir' : 'Admin',
      username: req.session.isCashier ? (req.session.cashierUsername || 'Kasir') : (req.session.adminUser || 'admin')
    };
